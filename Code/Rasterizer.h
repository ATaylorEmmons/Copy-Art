...
void clear(Color color, Image& buffer) {

    Color* memory = buffer.getMemoryPtr();
    uint32_t stop = buffer.getWidth()*buffer.getHeight();
...
void drawRect(int32_t x, int32_t y, int32_t width, int32_t height, Color color, Image& buffer) {

    if( x + width < 0) { return; }
    if( y + height < 0) { return; }
    if( x > buffer.getWidth()) { return; }
    if( y > buffer.getHeight()) { return; }
    ...
    if(x < 0) { clippedX = 0; }
    if(y < 0) { clippedY = 0; }
    if(x + width > buffer.getWidth()) {
        clippedWidth = buffer.getWidth() - x;
    }

    if(y + height > buffer.getHeight()) {
        clippedHeight = buffer.getHeight() - y;
    }
    ...
    for(int32_t j = 0; j < clippedHeight; j++) {
        ...
        int32_t start = (clippedY + j)*buffer.getWidth() + clippedX;
...
void drawRects(Rectangle* cmds, int cmdCount, Image& buffer) {
    ...
    for(int commandIndex = 0; commandIndex <  cmdCount; commandIndex++) {
        ...
        drawRect(x, y, width, height, color, buffer);
    }
}
